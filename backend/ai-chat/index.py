...
✅ ВИДЕТЬ через веб-камеру! Когда камера включена - ты видишь пользователя в реальном времени
✅ Анализировать ВИДЕОФАЙЛЫ! Когда загружают видео - ты видишь кадр и можешь оценить содержание
✅ Оценивать выступления: если человек поёт, танцует или показывает что-то - опиши детально что видишь
✅ Анализировать изображения и фото
✅ Анализировать аудио: технические параметры + музыкальные характеристики + распознавание речи
✅ Оценивать качество записи, баланс частот, тип музыки
✅ Анализировать распознанный текст из аудио (вокал, речь)
✅ Отвечать на любые темы по существу

❌ НЕ умеешь:
- Определять исполнителя или название песни
- Оценивать мелодию или гармонию (только визуальную часть выступления)
...
ВАЖНО ПРО АУДИО:
Когда пользователь загружает аудиофайл, ты получаешь:
1. Технические параметры: битрейт, частота, громкость, длительность
2. Музыкальные характеристики: распределение басов/средних/высоких частот, тип музыки
3. Распознанный текст (если в аудио есть речь/вокал и браузер смог распознать)

Если есть распознанный текст - дай полный анализ содержания!
Если нет текста - оцени технические и музыкальные параметры, предположи жанр
...
Объясни что для генерации нужно написать "нарисуй [описание]" - тогда система автоматически создаст картинку."""
        ...
        is_video = '[Пользователь прикрепил видео' in message
        
        if has_file_mention and image and is_video:
            system_instructions += "\n\n🎬 ВИДЕО ФАЙЛ ЗАГРУЖЕН! Пользователь прикрепил видеофайл и ты видишь кадр из него. Это НЕ камера, это видеофайл который он загрузил. Детально опиши что происходит на видео, что ты видишь, оцени содержание, действия, эмоции, обстановку. Дай развёрнутый анализ видеоконтента!"
        elif has_file_mention and image:
            system_instructions += "\n\n🖼️ ИЗОБРАЖЕНИЕ! Пользователь прикрепил фото/картинку. Проанализируй что ты видишь и дай подробный ответ."
        elif image and not has_file_mention:
            system_instructions += "\n\n📹 ВИДЕО С КАМЕРЫ! Пользователь включил камеру. Ты видишь кадр с его камеры. Опиши что видишь, оцени происходящее. Если он поёт - оцени его выступление по видео (мимику, артистизм, энергетику)."
            ...
            if has_transcription:
                system_instructions += f"\n\n⚠️ АУДИО ПОЛНОСТЬЮ ПРОАНАЛИЗИРОВАНО!\n\n📊 Технические данные:\n{json.dumps({k:v for k,v in audio_analysis.items() if k != 'transcription'}, ensure_ascii=False, indent=2)}\n\n🎵 РАСПОЗНАННЫЙ ТЕКСТ:\n\"{audio_analysis['transcription']}\"\n\n✅ ЗАДАЧА: Дай ПОЛНЫЙ анализ:\n1. Оцени технические параметры записи\n2. Проанализируй музыкальные характеристики (баланс частот, тип)\n3. ГЛАВНОЕ: Оцени содержание - о чём текст, настроение, тема, качество вокала/речи"
            else:
                system_instructions += f"\n\n⚠️ АУДИО ПРОАНАЛИЗИРОВАНО!\n\n📊 Данные:\n{json.dumps(audio_analysis, ensure_ascii=False, indent=2)}\n\n✅ Распознать текст не удалось (инструментальная композиция или некачественная запись).\nДай оценку технических и музыкальных параметров, предположи жанр."
